...
// Delete method
function* handle_delete_party_data(action) {
    console.log(action, "manage");
    try {
        let { status, data } = yield call(deleteData, action);
        if (status == 200) {
        ...
        yield put({ type: DELETE_PARTY_FAILED, error });
    }
}
...
export { handle_get_party_data, handle_add_vote_data, handle_get_vote_data, handle_get_election_data, handle_get_user_data, handle_add_election_data, handle_add_party_data, handle_get_connection_data, handle_add_connection_data, handle_add_user_data, handle_delete_party_data,handle_delete_election_data }